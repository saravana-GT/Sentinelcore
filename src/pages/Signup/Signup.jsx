...
function Signup() {
  ...
  const validate = (field, value) => { /* keep your validate same */ 
    let tempErrors = { ...errors };
    if (field === "username") {
      if (!value) tempErrors.username = "Username is required";
      else if (value.length < 3) tempErrors.username = "Username must be at least 3 characters";
  ...
  const handleSignup = async (e) => { /* keep your handleSignup same */
    e.preventDefault();
    setApiError("");
    setApiSuccess("");
    const tempErrors = {};
    ...
    try {
      ...
      if (response.ok) {
        ...
        auditLogs.unshift({ timestamp: new Date().toLocaleTimeString(), user: username, action: `User registered with role: ${role}` });
        localStorage.setItem("audit_logs", JSON.stringify(auditLogs));
        setTimeout(() => { navigate("/"); }, 2000);
      } else { setApiError(data.error || "Registration failed."); }
    } catch (err) { setApiError("Network error. Make sure the backend is running."); } 
    ...
    <div className="signup-container">
      <div className="signup-card">
        ...
        <form onSubmit={handleSignup}>
          ...
          <input type="text" value={username} onChange={(e) => {setUsername(e.target.value); validate("username", e.target.value);}} placeholder="Enter Username" className={errors.username ? "input-error" : ""} />
          {errors.username && <span className="error-text">{errors.username}</span>}

          <label>Email Address</label>
          <input type="email" value={email} onChange={(e) => {setEmail(e.target.value); validate("email", e.target.value);}} placeholder="Enter Email Address" className={errors.email ? "input-error" : ""} />
          {errors.email && <span className="error-text">{errors.email}</span>}

          <label>Password</label>
          <input type="password" value={password} onChange={(e) => {setPassword(e.target.value); validate("password", e.target.value);}} placeholder="Enter Password" className={errors.password ? "input-error" : ""} />
